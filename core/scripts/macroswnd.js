...
(function (TypeofObjects) {
    TypeofObjects[TypeofObjects["Macros"] = 0] = "Macros";
    TypeofObjects[TypeofObjects["Addin"] = 1] = "Addin";
    TypeofObjects[TypeofObjects["Group"] = 2] = "Group";
...
var RowInfo = (function () {
    function RowInfo(type, _a) {
        ...
        this.type = type;
        this.info = { picture: undefined, hotkey: "", descr: "" };
        this.addin = addin;
        this.macros = macros;
        this.group = group;
    }
    RowInfo.prototype.key = function () {
...
exports.MacrosWnd = (function () {
    var MacrosWnd = (function () {
        function MacrosWnd() {
            ...
            var self = this;
            this.tc = new stdlib.TextChangesWatcher(this.form.Controls.Pattern, 3, function (pattern) { return self.update(pattern); });
            events.connect(Designer, "onLoadAddin", this, "onChangeAddin");
            events.connect(Designer, "onUnLoadAddin", this, "onChangeAddin");
        }
        MacrosWnd.prototype.isOpened = function () {
            return this.form.IsOpen();
        };
        MacrosWnd.prototype.onChangeAddin = function () {
            this.filled = false;
        };
        MacrosWnd.prototype.processAddinsGroup = function (rows, group) {
            ...
            for (var i = 0, count = group.addinsCount; i < count; i++) {
                ...
                if (this.mode == TypeofObjects.Addin) {
                    ...
                    if (!macroses.length)
                        continue;
                    var parentRows; // Сюда будем вставлять макросы
                    ...
                    for (var m in macroses) {
                        ...
                        var macrosRow = ins.rows.Добавить();
        ...
        MacrosWnd.prototype.update = function (pattern) {
            pattern = pattern.replace(/\s+/g, ' ').replace(/^\s|\s$/g, '');
            if (pattern.length)
                this.pattern = pattern.toLowerCase().split(' ');
            else
                this.pattern = null;
            var currentRow = this.MacrosTreeCtrl.ТекущаяСтрока;
        ...
        MacrosWnd.prototype.CmdsСделатьВыбор = function () {
            var tp = this.MacrosTreeCtrl;
            if (!tp.ТекущаяСтрока)
                return;
            var selected = tp.ТекущаяСтрока.rowInfo;
        ...
        MacrosWnd.prototype.MacrosTreeВыбор = function () {
            this.CmdsСделатьВыбор();
        };
        MacrosWnd.prototype.ПриОткрытии = function () {
            this.tc.start();
        };
        MacrosWnd.prototype.ПриЗакрытии = function () {
            this.tc.stop();
        };
        MacrosWnd.prototype.MacrosTreeOnRowOutput = function (Control, RowAppearance, RowData) {
            var rowInfo = RowData.val.rowInfo;
